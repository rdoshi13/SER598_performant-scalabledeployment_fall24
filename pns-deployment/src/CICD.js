    ...
    <div className="CICD">
      ...
      <section>
        ...
        <ol>
          ...
          <pre><code>docker exec my-jenkins cat /var/jenkins_home/secrets/initialAdminPassword</code></pre>
      ...
      <section>
        ...
        <ol>
          ...
          <pre><code>echo "Hello Jenkins"</code></pre>
      ...
      <section>
        ...
        <ol>
          ...
          <p>Once the application is running, you can access it at the provided URL.</p>
        </ol>
      </section>
...
{'}'}
      ...
      <section>
        ...
        <p>To ensure NPM is available, use Docker to provide the required environment. Follow these steps:</p>
        <ol>
          <li>Install the “Docker Pipeline” plugin from the Jenkins dashboard under Manage Jenkins → Plugins.</li>
          <li>
            Just checking if npm is installed on the agent, and if you see the logs you will find npm is not found.
            Now to solve this issue we can do either of 2 things:
            <ol type="a">
              <li>One is to simply go to the agent and manually install NPM</li>
              <li>Other option is to use docker, which allows us to use a container, so let's learn how to use docker</li>
            </ol>
            Go to dashboard, then manage jenkins, and then go to plugins then go to available plugins and install “docker pipeline” plugin. Jenkins is a plugin monster, it's a skeleton which is to be fed with various plugins to make it effective.
          </li>
        </ol>
        <p>
pipeline {'{'}<br />
...
{'}'}
      ...
      <section>
        ...
        <p>After build, it will be successful. Docker will be identified as an agent, it will pull the npm image if not present already and it will echo and print the statement and npm version and then the job finishes.</p>

        <p>Next, we want to define a Jenkins pipeline but in the project itself.</p>
        <p>So now go to the <strong>learn-jenkins-app</strong> repository which we cloned earlier from our forked GitHub repo and create a new file called <code>Jenkinsfile</code>. It has to be written exactly that way.</p>
        <p>Then we can go to Jenkins portal, use an existing job and script and copy (use hello world script):</p>
        <pre><code>
pipeline {'{'}
    agent any
...
{'}'}
        ...
        <p>But how can we get this to run in Jenkins? Now we have to create a new item, then create a new pipeline. Now after going to pipeline section of the new job, we will select <strong>pipeline script from scm</strong>. SCM means source code management, basically telling the agent that we want to get the script from somewhere else. Select SCM as git, and put your forked project url in the url field (Code → HTTP → copy address), no need for any credentials as this is a public repo and Jenkins will be able to read this. Change branch specifier to <code>*/main</code>. Now save it and build, and you will see a successful build of the pipeline.</p>
      </section>

      <section>
        <h2>Building the Application with Jenkins</h2>
        <p>Now we shall see how to build this application. In the case of this application, we need to create a production build where essential files need to be minimized and many other steps.</p>

        <p>Now we shall come to our Jenkinsfile and rename the stage to ‘build’. We also need to use a docker image with node installed so as to run npm. We will use <code>node:18-alpine</code> as it’s much smaller and compact. Let's also do a <code>sh 'ls -la'</code> to understand what has happened in the workspace before and after.</p>
        
        <p>Instead of <code>npm install</code> we will use <code>npm ci</code> (designed for CI/CD servers) and then <code>npm run build</code>.</p>
...
{'}'}
        ...
        <p>If you hit build now for this job, you will see the exact output in the console output that you would have seen in the case of running <code>npm build</code> in your terminal, which is "compiled successfully".</p>
      </section>
      ...
      {/* Add the "Test your knowledge" button here */}
